CLASS zcl_zip_diff_item DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES : BEGIN OF ENUM enum_diff_state STRUCTURE state,
            ...
            END OF ty_diff_item,
            ty_diff_items TYPE HASHED TABLE OF ty_diff_item WITH UNIQUE KEY local_name.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_items_at_path
      IMPORTING
        ...
        path              TYPE string OPTIONAL
      RETURNING
        VALUE(path_items) TYPE string_table.
...
CLASS ZCL_ZIP_DIFF_ITEM IMPLEMENTATION.
  ...
  METHOD compare_items_at_path.
    ...
    WHILE zip_1_read = abap_true OR zip_2_read = abap_true.
      ...
      IF zip_1_read = abap_true.
        ...
        ELSE.
          DATA(item_1) = REF #( zip_1_items[ zip_1_index ] OPTIONAL ).
        ENDIF.
      ENDIF.
      IF zip_2_read = abap_true.
        ...
        ELSE.
          DATA(item_2) = REF #( zip_2_items[ zip_2_index ] OPTIONAL ).
        ENDIF.
      ENDIF.
      ...
      IF zip_1_index <= lines( zip_1_items ) AND zip_2_index <= lines( zip_2_items ).
        ...
        ELSE.
          ...
          zip_1_read = abap_true.
          zip_2_read = abap_true.
          IF contains( val = item_1->* end = '/' ).
            " folder
            folder_diff = compare_items_at_path( path = |{ path }{ item_1->* }| ).
            ...
            ENDIF.
          ELSE.
            diff_state = compare_two_items( zip_1 = zip_1 zip_2 = zip_2 name_1 = |{ path }{ item_1->* }| name_2 = |{ path }{ item_2->* }| ).
          ENDIF.
        ENDIF.
      ...
      diff_item = VALUE #(
          LET local_name = SWITCH #( diff_state WHEN state-added THEN item_2->* ELSE item_1->* ) IN
  ...
  METHOD get_items_at_path.
    ...
    LOOP AT items ASSIGNING FIELD-SYMBOL(<item>).
      IF path = ``.
        starter = substring_to( val = <item>-name regex = item_or_folder ).
        COLLECT starter INTO path_items.
      ELSEIF contains( val = <item>-name start = path ).
        starter = substring_to( val = substring( val = <item>-name off = strlen( path ) ) regex = item_or_folder ).
        COLLECT starter INTO path_items.
      ENDIF.
    ENDLOOP.

    SORT path_items BY table_line.
